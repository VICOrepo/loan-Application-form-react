...
const validationSchema = Yup.object().shape({
  ...
  HPhone: Yup.string().test("len", "*Field shouldn't be empty", (val = "") => {
    const val_length_without_dashes = val.replace(/-|_(|_)/g, "").length;
    return val_length_without_dashes === 12;
  }),

  MPhone: Yup.string().test("len", "*Field shouldn't be empty", (val = "") => {
    const val_length_without_dashes = val.replace(/-|_(|_)/g, "").length;
    return val_length_without_dashes === 12;
  }),

  SSN: Yup.string().test("len", "*Field shouldn't be empty", (val = "") => {
    const val_length_without_dashes = val.replace(/-|_(|_)/g, "").length;
